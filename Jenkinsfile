  ...
  containers:
    ...
    - command:
      ...
      env:
        ...
        - name: "LANG"
          value: "en_US.UTF-8"
      image: "registry.cn-beijing.aliyuncs.com/tech_cicd/maven:3.5.3"
      imagePullPolicy: "IfNotPresent"
      name: "build"
      tty: true
      volumeMounts:
    ...
    stage('pulling Code') {
      parallel {
        stage('pulling Code') {
          ...
          steps {
            git(branch: "${BRANCH}", credentialsId: '304b8f40-70ef-4454-ac3e-bf61023ebf51', url: "${REPO_URL}")
          }
        }
        ...
        stage('pulling Code by trigger') {
          ...
          steps {
            git(url: "${REPO_URL}", branch: env.gitlabBranch, credentialsId: '304b8f40-70ef-4454-ac3e-bf61023ebf51')
          }
        }
